...
ordered_importance = sorted(zip(rf_model.feature_importances_, X.columns), reverse=True)
ordered_importance_df = pd.DataFrame(data=ordered_importance, columns=['Feature_Importance', 'Feature_Name'])
ordered_importance_df

# Print the top five "important" features
print("The top five features from this model:")
for i in range(0,5):
    print(f'Feature: {ordered_importance_df.Feature_Name[i]}, importance: {ordered_importance_df.Feature_Importance[i]}')